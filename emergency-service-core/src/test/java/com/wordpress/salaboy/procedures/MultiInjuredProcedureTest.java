...
public class MultiInjuredProcedureTest extends GridBaseTest {
	...
	@Test
	public void defaultHeartAttackSimpleTest() throws HornetQException,
		...
		ProceduresMGMTService.getInstance().newRequestedProcedure(emergency.getId(), "MultiInjuredPeopleProcedure", parameters);

		Thread.sleep(5000);

		List<TTaskAbstract> taskAbstracts = humanTaskServiceClient
				.getMyTaskAbstracts("", "garage_emergency_service", "", null,
						"", "", "", 0, 0);
		Assert.assertNotNull(taskAbstracts);
		Assert.assertEquals(1, taskAbstracts.size());
		TTaskAbstract taskAbstract = taskAbstracts.get(0); // getting the first
															// task
		Assert.assertEquals(" Select Vehicle For " + call.getId() + " ",
				taskAbstract.getName().getLocalPart());

		// I need to get the Content Data and check the values of the Emergency
		// and Call Ids.
		// Using that I need to select one vehicle ID from the list of all the
		// vehicles.

		TTask task = humanTaskServiceClient.getTaskInfo(taskAbstract.getId());
		Assert.assertNotNull(task);

		humanTaskServiceClient
				.setAuthorizedEntityId("garage_emergency_service");
		humanTaskServiceClient.start(task.getId());

		List<TAttachmentInfo> attachmentsInfo = humanTaskServiceClient
				.getAttachmentInfos(task.getId());
		TAttachmentInfo firstAttachmentInfo = attachmentsInfo.get(0);
		TAttachment attachment = humanTaskServiceClient.getAttachments(
				task.getId(), firstAttachmentInfo.getName()).get(0);

		String value = (String) ((Map) attachment.getValue()).get("Content");

		Assert.assertNotNull(value, "1,1");

		Map<String, Object> info = new HashMap<String, Object>();
		List<Vehicle> vehicles = new ArrayList<Vehicle>();
		Ambulance ambulance = new Ambulance("My Ambulance", new Date());
		String ambulanceId = ContextTrackingServiceImpl.getInstance()
				.newVehicle();
		ambulance.setId(ambulanceId);
		Ambulance ambulance2 = new Ambulance("My Ambulance2", new Date());
		String ambulanceId2 = ContextTrackingServiceImpl.getInstance()
				.newVehicle();
		...
		ProceduresMGMTService.getInstance().notifyProcedures(
				new VehicleHitsEmergencyMessage(ambulanceId, emergency.getId(),
						new Date()));
		ProceduresMGMTService.getInstance().notifyProcedures(
				new VehicleHitsEmergencyMessage(ambulanceId2, emergency.getId(),
						new Date()));

		Thread.sleep(4000);

		humanTaskServiceClient.setAuthorizedEntityId("doctor");
		taskAbstracts = humanTaskServiceClient.getMyTaskAbstracts("", "doctor",
				"", null, "", "", "", 0, 0);
		Assert.assertNotNull(taskAbstracts);
		Assert.assertEquals(2, taskAbstracts.size());
		...
		attachmentsInfo = humanTaskServiceClient.getAttachmentInfos(task
				.getId());
		firstAttachmentInfo = attachmentsInfo.get(0);
		attachment = humanTaskServiceClient.getAttachments(task.getId(),
				firstAttachmentInfo.getName()).get(0);
		...
		ProceduresMGMTService.getInstance().notifyProcedures(
				new VehicleHitsHospitalMessage(ambulanceId2, new Hospital(
