 ...
 * @author salaboy
 */
public class MultiInjuredProcedureNoSmartTasksTest extends GridBaseTest {
    ...
    private String ambulanceId2;

    public MultiInjuredProcedureNoSmartTasksTest() {
    }
    ...
    public void defaultHeartAttackSimpleTest() throws HornetQException, InterruptedException, IOException, ClassNotFoundException {
        ...
        //The vehicle reaches the emergency
        ProceduresMGMTService.getInstance().notifyProcedures(new VehicleHitsEmergencyMessage(ambulanceId2, emergency.getId(), new Date()));

        Thread.sleep(4000);


        //The vehicle reaches the hospital
        ProceduresMGMTService.getInstance().notifyProcedures(new VehicleHitsHospitalMessage(ambulanceId2, new Hospital("Hospital B", 1, 0), emergency.getId(), new Date()));
...
//        Assert.assertEquals(1, emergency.getUpdatesForVehicle(ambulanceId2).size());
    ...
    private void doGarageTask() throws IOException, ClassNotFoundException {
        ...
        ambulanceId = ContextTrackingServiceImpl.getInstance().newVehicle();
        ambulance.setId(ambulanceId);
        vehicles.add(ambulance);
        
        Ambulance ambulance2 = new Ambulance("My Ambulance 2", new Date());
        ambulanceId2 = ContextTrackingServiceImpl.getInstance().newVehicle();
...
//        ContextTrackingServiceImpl.getInstance().attachVehicle(procedureId, ambulanceId);
    ...
    private void doDoctorTask() throws IOException, ClassNotFoundException {
        ...
        List<TaskSummary> sums = handler.getResults();
        assertNotNull(sums);
        assertEquals(false, sums.isEmpty());
        ...
        long anumber = System.currentTimeMillis();
        info.put("comment", "**Comment " + anumber);
        info.put("priority", 2);
        ContentData result = new ContentData();
        result.setAccessType(AccessType.Inline);
        result.setType("java.util.Map");
